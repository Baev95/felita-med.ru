        ...
        <div class="reviews-page__sort">
            <div class="reviews-page__sort_block sort_block">
                ...
                <select id="select-services" name="reviews-services" class="filter-select select" data-filter-type="service">
                    ...
                    <?
                    $sections = CIBlockSection::GetList(['SORT' => 'ASC'], ['IBLOCK_ID' => 5, 'ACTIVE' => 'Y'], true, ['CODE', 'NAME']);
                    while ($arItem = $sections->GetNext()) echo "<option value='{$arItem['CODE']}'>{$arItem['NAME']}</option>";
                    ?>
                </select>
            </div>
            <div class="reviews-page__sort_block sort_block">
                ...
                <select id="select-doctor" name="reviews-doctor" class="filter-select select" data-filter-type="doctor">
                    <option value="all" selected><?= GetMessage('SELECT_ALL') ?></option>
                    <? $res = CIBlockElement::GetList(['ID' => 'ASC'], ['IBLOCK_ID' => 3], false, false, ['NAME', 'CODE']);
                    while ($row = $res->Fetch()) echo "<option value='{$row['CODE']}'>{$row['NAME']}</option>"; ?>
                </select>
            </div>
        </div>
        ...
        <div class="reviews-page__cards">
            <? foreach ($arResult["ITEMS"] as $arItem) : ?>
                ...
                if ($arItem['TIMESTAMP_X']) :
                    $dateCreate = CIBlockFormatProperties::DateFormat('j F Y', MakeTimeStamp(($arItem['TIMESTAMP_X']), CSite::GetDateFormat()));
                endif;
                $res = CIBlockElement::GetList(['ID' => 'ASC'], ['IBLOCK_ID' => 3, 'ID' => $arItem['PROPERTIES']['DOCTOR']['VALUE']], false, false, ['CODE', 'NAME']);
                while ($row = $res->Fetch()) :
                    $doctor_code = $row['CODE'];
                    $doctor_name = $row['NAME'];
                endwhile;
                $sections = CIBlockSection::GetList(['SORT' => 'ASC'], ['IBLOCK_ID' => 5, 'ACTIVE' => 'Y'], true, ['CODE', 'NAME']);
                while ($row = $sections->GetNext()) :
                    $services_code = $row['CODE'];
                endwhile;
                ?>
                <div class="reviews__card reviews-page__card filter-card quantity-card" data-service="<?= $services_code ?>" data-doctor="<?= $doctor_code ?>">
                    <div class="reviews__card_inner">

                        <div class="reviews__card_top">
                            <div class="reviews__card_fio">
                                <p><?= $arItem['NAME'] ?>, <?= $arItem['PROPERTIES']['AGE']['VALUE'] . " " . $declension->get($arItem['PROPERTIES']['AGE']['VALUE']); ?></p>
                                <p><?= $arItem['PROPERTIES']['CITY']['VALUE'] ?></p>
                            </div>

                            <? if ($arItem['PROPERTIES']['STARS']['VALUE']) : ?>
                                <div class="reviews__card_rating">
                                    <div class="reviews__card_stars">
                                        <?
                                        for ($i = 0; $i < 5; $i++) :
                                            $star = $i < $arItem['PROPERTIES']['STARS']['VALUE'] ? 'reviews__card_star-active' : 'reviews__card_star';
                                            echo <<<OED
												<span class="$star">
                    ...
                    <div class="reviews__card_bottom">
                        <p><?= $arItem['PROPERTIES']['TEXT_DOCTOR']['VALUE']['TEXT'] ?></p>
